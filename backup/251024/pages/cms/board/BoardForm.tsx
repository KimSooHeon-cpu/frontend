...
const BoardFormPage: React.FC = () => { // 함수형 컴포넌트 선언
  ...
  const handleSave = async () => { // 저장 버튼 클릭 시 실행
    ...
    try { // 통신 시도
      // 전송용 데이터 객체 생성 (application/x-www-form-urlencoded)
      ...
      */
      ...
      //! FAIL 알림 → 등록/수정 구분 문구 출력
      if (res.data?.success === false) { // 만약 성공 여부가 false일 경우
  ...
  return ( // 화면 렌더링 시작
    ...
    <div style={{ width: "100%", maxWidth: "100%", margin: "30px auto", color: "#222" }}>{/* 컨테이너 */}
