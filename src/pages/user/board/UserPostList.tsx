...
  const pageRange = 5; // 한 번에 표시할 페이지 수
  ...
  const loadBoardData = async () => {
    ...
    try {
      ...
      // [251110] Navbar에서 boardId(PK)를 넘겨주므로, boardNum이 아닌 boardId로 비교하도록 수정
      const foundBoard = allBoards.find((b) => String(b.boardId) === boardId && b.boardUse === "Y");
      ...
      if (foundBoard) {
        ...
        // [251110] 백엔드 페이징 응답(Page 객체)에 맞춰 실제 데이터는 .content에서 가져오도록 수정
        if (postsRes.data && Array.isArray(postsRes.data.content)) {
          setPosts(postsRes.data.content);
        } else {
          // 페이징 객체가 아닌 일반 배열로 올 경우를 대비한 방어 코드
